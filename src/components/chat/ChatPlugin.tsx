        ...
        >
          {isOpen ? <X className="w-6 h-6" /> : (
            <div className="relative">
              ...
              {unreadCount > 0 && (
                <div className="absolute -top-2 -right-2 w-5 h-5 bg-red-500 rounded-lg flex items-center justify-center text-xs font-bold border-2 border-white">
                  {unreadCount}
                </div>
              )}
            </div>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-28 right-6 w-full max-w-[400px] h-[600px] max-h-[80vh] bg-white rounded-lg  border border-line-light flex flex-col overflow-hidden z-50"
          >
            {/* Header */}
            <div className="bg-slate-900 text-white p-4 flex items-center justify-between shrink-0">
              <div className="flex items-center gap-3">
                {activeChat && (
                  <button onClick={() => setActiveChat(null)} className="p-1 hover:bg-slate-800 rounded-lg transition-colors">
              ...
              {activeChat && (
                <button 
                  ...
                  className="p-2 text-ink-muted hover:text-red-400 transition-colors rounded-full"
            ...
            {/* Content Area */}
            <div className="flex-1 bg-paper-2 overflow-hidden flex flex-col">
              {!activeChat ? (
                // Chat List
                <div className="flex-1 overflow-y-auto p-4 space-y-2">
                  {chats.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center text-ink-muted space-y-4">
                    ...
                    chats.map(chat => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-ink-muted truncate font-medium mt-1">
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                    ...
                    return (
                      <div key={msg.id || i} className={cn("flex flex-col max-w-[80%]", isMe ? "ml-auto items-end" : "mr-auto items-start")}>
                        <div className={cn(
                          ...
                          {msg.text}
                          {msg.attachmentUrl && (
                            <img src={msg.attachmentUrl} alt="attachment" className="mt-2 rounded-lg max-w-full h-auto max-h-[150px] object-cover" />
                          )}
                        </div>
                      </div>
                    );
                  ...
                  {isUploading && (
                     <div className="mr-auto items-start flex flex-col max-w-[80%]">
                        <div className="p-3 rounded-lg text-sm  bg-white border border-line text-ink-muted rounded-tl-sm animate-pulse flex items-center gap-2">
            ...
            {/* Input Area */}
            {activeChat && (
              <div className="p-4 bg-white border-t border-line-light flex items-center gap-2 shrink-0">
                <input
                  type="file"
                  ref={fileInputRef}
