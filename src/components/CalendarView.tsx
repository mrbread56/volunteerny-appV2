      ...
      <div className="lg:col-span-8 space-y-8">
        ...
        <Card className="rounded-lg border border-[#1F4C63]/10  /50 /20 p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex gap-3.5 items-start">
            ...
            <div>
              <h3 className="font-semibold text-ink text-sm leading-snug flex items-center gap-2">
                Google Calendar Synchronization
                {isGoogleCalendarLinked && (
                  <span className="bg-emerald-500/10 text-emerald-600 border border-emerald-500/15 px-2 py-0.5 rounded-lg text-[8.5px] uppercase font-semibold tracking-widest leading-none">Connected</span>
                )}
              </h3>
              <p className="text-xs font-semibold text-ink-soft mt-1 max-w-lg">
        ...
        <Card className="rounded-lg border border-line  bg-white overflow-hidden p-6 md:p-8">
          <div className="flex items-center justify-between pb-6 border-b border-line">
            ...
            <div className="flex items-center gap-2 border border-line p-1 bg-paper-2/50 rounded-lg">
              <button 
                onClick={handlePrevMonth}
                className="w-10 h-10 flex items-center justify-center text-ink-soft hover:text-ink-soft hover:bg-white rounded-lg transition-all cursor-pointer outline-none rounded-full"
              >
          ...
          <div className="grid grid-cols-7 gap-1 md:gap-3 py-6 text-center">
            {/* Weekdays headers */}
            {weekdays.map((day) => (
              <span key={day} className="text-xs text-ink-soft font-semibold uppercase tracking-wider">
                {day}
              </span>
            ))}
            ...
            {Array.from({ length: startOffset }).map((_, idx) => (
              <div 
                key={`empty-${idx}`} 
                className="aspect-square bg-paper-2/40 border border-transparent rounded-lg opacity-30 select-none pointer-events-none" 
              />
            ))}
            ...
            {Array.from({ length: daysInMonth }).map((_, idx) => {
              ...
              return (
                <div
                  ...
                  className={cn(
                    "aspect-square rounded-lg md:rounded-lg border flex flex-col items-center justify-between p-2 cursor-pointer transition-all duration-200 relative select-none",
                    isSelected 
                ...
                >
                  ...
                  {dayHoursSum > 0 ? (
                    <span className={cn(
                      "text-[8px] md:text-xs font-semibold tracking-tighter block uppercase",
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-paper-2 rounded-lg p-4 text-xs text-ink-soft flex items-center gap-2 border border-line">
              <Info className="w-4 h-4 text-ink-soft shrink-0" />
              <span className="font-semibold text-ink-soft">
                <strong className="text-ink-soft font-bold">Logged Hours (Emerald):</strong> Confirmed volunteer credits and approved supervision reports.
              </span>
            </div>
            ...
            <div className="bg-paper-2 rounded-lg p-4 text-xs text-ink-soft flex items-center gap-2 border border-line">
              ...
              <span className="font-semibold text-ink-soft">
                <strong className="text-ink-soft font-bold">Upcoming Shift (Blue):</strong> Verified volunteer applications accepted by organizations.
              </span>
            </div>
          </div>
        </Card>
        ...
        <Card className="rounded-lg border border-line  bg-white overflow-hidden flex flex-col min-h-[460px]">
          ...
          <CardContent className="p-6 md:p-8 flex-1 flex flex-col justify-between">
            <div className="space-y-8">
              ...
              <div>
                <h4 className="text-xs uppercase font-semibold tracking-widest text-ink-soft mb-3 block">Upcoming Schedule</h4>
                {selectedDateOpps.length > 0 ? (
                    ...
                    {selectedDateOpps.map((opp: any) => (
                      <div key={opp.id} className="p-4 rounded-lg bg-[#1F4C63]/5/15 border border-[#1F4C63]/10/50 space-y-3 relative animate-fadeIn">
                        ...
                        <div className="pt-2 border-t border-line/40 space-y-2">
                          <p className="text-xs font-bold text-ink-soft uppercase tracking-wider">Sync Shift to Calendar</p>
                          <div className="grid grid-cols-2 gap-2">
                            ...
                            <button
                              ...
                              className="h-8 rounded-lg bg-paper-2 hover:bg-paper-3 border border-line text-ink-soft font-semibold flex items-center justify-center gap-1.5 text-xs uppercase tracking-wide px-2 cursor-pointer"
                            >
                              <Download className="w-3 h-3 text-ink-soft" /> Apple
                            </button>
                  ...
                  <p className="text-xs font-semibold text-ink-soft italic">No scheduled upcoming sessions for this date.</p>
                )}
              </div>

              {/* SECTION: Logged Core Activities */}
              <div>
                <h4 className="text-xs uppercase font-semibold tracking-widest text-ink-soft mb-3 block">Logged Credits</h4>
                {selectedDateLogs.length > 0 ? (
                  <div className="space-y-3">
                    {selectedDateLogs.map((log: any) => (
                      <div key={log.id} className="p-4 rounded-lg bg-emerald-50/20 border border-emerald-100/50 space-y-2 relative animate-fadeIn">
                        <div className="flex items-start justify-between gap-2">
                          <div>
                            <p className="text-xs font-bold text-ink">{log.activity}</p>
                            <span className="text-[8.5px] text-ink-soft font-bold font-mono">ID: {log.id}</span>
                          </div>
                          <span className="text-xs font-semibold text-emerald-700 bg-emerald-50 border border-emerald-100 py-0.5 px-2 rounded-lg shrink-0">
                            {log.hours} hours
                          </span>
                        </div>

                        <div className="border-t border-line pt-2 text-xs text-ink-soft font-semibold space-y-1">
                          <p className="flex items-center gap-1.5 leading-tight">
                            <span className="w-1.5 h-1.5 bg-slate-400 rounded-lg" /> Supervisor: {log.coordinatorName}
                          </p>
                  ...
                  <p className="text-xs font-semibold text-ink-soft italic">No credit logging coordinates stored on this date.</p>
                )}
              </div>

            </div>

            <div className="pt-6 border-t border-line-light mt-6 shrink-0">
              <div className="p-4 bg-paper-2 border border-line rounded-lg text-center space-y-2">
                <p className="text-xs font-semibold uppercase text-ink-soft tracking-wider">
