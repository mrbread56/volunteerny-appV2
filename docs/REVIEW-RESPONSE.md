...
| R14 | Prioritise new features by version | ✅ | Same file. Architecture work is intentionally placed **ahead** of every feature |
...
| R33 | Component structure is acceptable. Calendar, ApplicationReview, Referral separated | ✅ | Agreed on the structure. The unrendered `CalendarView.tsx` (827 lines) and the `chats`/`messages` rules that had no code were both deleted as requested, dead permissions are untested attack surface, and the component had nine declared-but-never-read state variables for a quick-log feature that was never built |
